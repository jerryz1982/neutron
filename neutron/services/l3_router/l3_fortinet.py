...
class FortinetL3ServicePlugin(router.L3RouterPlugin):
    ...
    def create_floatingip(self, context, floatingip):
        ...
        """
        returned_obj = super(FortinetL3ServicePlugin, self).create_floatingip(
            context, floatingip)
        try:
    ...
    def _associate_floatingip(self, context, id, floatingip):
        ...
        tenant_id = self._get_floatingip(context, id).tenant_id
        ...
        if record:
            if record.allocated and record.bound:
                ...
                raise Exception("The floating ip %s already used"
                                % floating_ip_address)
            if record.allocated and not record.bound:
